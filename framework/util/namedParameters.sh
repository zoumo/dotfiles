...
Variable::TrapAssignNumberedParameter() {
	...
	if [[ "$command" == "trap" || "$command" == "l="* || "$command" == "_type="* || "$command" == "_isRequired="* || "$command" == "_isReadOnly="* || "$command" == "_noHandle="* || "$command" == "_isGlobal="* ]]; then
		return 0
	fi
	...
	if [[ $varDeclaration == '-'* || $varDeclaration == '${__assign'* ]]; then
	...
	if [[ "${__assign_varType:-null}" != "null" ]]; then
		...
		if [[ "$__assign_valueRequired" == 'true' && -z "${!__assign_paramNo+x}" ]]; then
		...
		if [[ "${!indirectAccess-}" == "$var:"* ]]; then
			local realVarName="${!indirectAccess#*$var:}"
			if Variable::Exists "$realVarName"; then
				local __declaration
				...
				if [[ "$__declaration_type" != "$__assign_varType" && "$__assign_varType" != 'params' && "$__assign_varType" != 'rest' ]]; then
		...
		case "$__assign_varType" in
			...
			'boolean')
				...
				if [[ ! -z "${!indirectAccess-}" ]]; then
					if [[ "${!indirectAccess}" == "${boolean_fingerprint}"* ]]; then
						__assign_varValue="${!indirectAccess}"
					elif [[ "${!indirectAccess}" == 'true' || "${!indirectAccess}" == 'false' ]]; then
						__assign_varValue="${boolean_fingerprint}${!indirectAccess}"
					else
						__assign_varValue="${boolean_fingerprint}false"
					fi
				elif [[ "${__assign_varValue}" == 'true' || "${__assign_varValue}" == 'false' ]]; then
					__assign_varValue="${boolean_fingerprint}${__assign_varValue}"
				elif [[ "${__assign_varValue}" != "${boolean_fingerprint}true" && "${__assign_varValue}" != "${boolean_fingerprint}false" ]]; then
					__assign_varValue="${boolean_fingerprint}false"
				fi
				eval "$__assign_varName=\"${__assign_varValue}\""
				;;
			'string' | 'integer' | 'reference')
				if [[ "$__assign_varType" == "reference" || ! -z "${!indirectAccess-}" ]]; then
					if [[ "${!indirectAccess}" == "$ref:"* ]]; then
		...
		if [[ "$__assign_noHandle" != 'true' && ! -z ${__oo__bootstrapped+x} ]] && declare -f 'Type::CreateHandlerFunction' &>/dev/null; then
			DEBUG Log "Will create handle for $__assign_varName"
			Type::CreateHandlerFunction "$__assign_varName" # 2> /dev/null || true
		fi
	fi

	if [[ "$command" != "local" || "${__assign_next-}" != "true" ]]; then

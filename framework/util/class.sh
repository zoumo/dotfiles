...
Type::Construct() {
    ...
    if [[ ! -z "${__constructor_recursion+x}" ]]; then
    ...
    if Variable::Exists "__${typeSanitized}_property_names"; then
        local propertyIndexesIndirect="__${typeSanitized}_property_names[@]"
        local -i propertyIndex=0
        local propertyName
        for propertyName in "${!propertyIndexesIndirect}"; do
            ...
            if [[ $propertyType == 'boolean' ]] && [[ "$defaultValue" == 'false' || "$defaultValue" == 'true' ]]; then
            ...
            if [[ ! -z "$constructedPropertyDefinition" ]]; then
    ...
    if [[ -z "$assignToVariable" ]]; then
        Variable::PrintDeclaration constructedType
    else
        local constructedIndex
        for constructedIndex in "${!constructedType[@]}"; do

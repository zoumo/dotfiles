...
Type::GetPrimitiveExtensionFromVariable() {
	...
	if [[ "${!variableName}" != "$__primitive_extension_declaration"* ]]; then
		return
	fi

	local prefix=__primitive_extension_fingerprint__
	local extensionType
	for extensionType in $(Variable::GetAllStartingWith $prefix); do
		local fingerprint=${!extensionType}
		if [[ "${!variableName}" == "$fingerprint"* ]]; then
...
Type::CreateHandlerFunction() {
	...
	if ! Command::Exists "${__oo__variableMethodPrefix}${variableName}"; then
	...
	elif ! Array::Contains "${variableName}" "${__oo__type_handler_functions[@]}"; then
...
Type::RunFunctionGarbageCollector() {
	...
	local index
	local handler
	for index in "${!__oo__type_handler_functions[@]}"; do
...
Type::TrapAndCreate() {
	...
	if [[ "$command" == "trap" || "$command" == "l="* || "$command" == "_type="* ]]; then
		# set +x
		return 0
	fi

	if [[ "${commandWithArgs[*]}" == "true" ]]; then
	...
	if [[ "$varValue" == "$varName" ]]; then
		# Log "equal $varName=$varValue"
		local varValue=""
	fi

	if [[ ! -z $__typeCreate_varType ]]; then
		...
		if [[ -z "$__typeCreate_varValue" ]]; then
			...
			case "$__typeCreate_varType" in
				'boolean')
					if [[ "${__typeCreate_varValue}" != 'true' && "${__typeCreate_varValue}" != 'false' ]]; then
	...
	if [[ "$command" != "declare" || "$__typeCreate_next" != "true" ]]; then
...
@return() {
	...
	if [[ "${__local_return_self_and_result}" == "true" || "${__return_self_and_result}" == "true" ]]; then
		# Log "returning heavy"
	...
	elif [[ "${#__return_declaration}" -gt 0 ]]; then
...
Type::RunCurrentStack() {
	...
	if [[ ! -z "$returnValueDefinition" && "$affectTheInitialVariable" == 'false' ]]; then
	...
	local resultString=$(__return_self_and_result=true Type::ExecuteMethod "$type" "$variableName" "$method" "${params[@]}" || {
		local falseBool="${__primitive_extension_fingerprint__boolean}:false"
		__return_self_and_result=true @return falseBool $variableName
	})
	...
	if [[ -z "$resultString" || "$resultString" != *"$__return_separator"* ]]; then
	...
	if [[ "$assignResult" != "${returnValueDefinition}" ]]; then
...
Type::RunGetter() {
	...
	if Function::Exists "$type.__getter__"; then
...
Type::Handle() {
	...
	if [[ $# -gt 0 ]]; then
		...
		while [[ $# -gt 0 ]]; do
			if [[ "$__access_private" != "true" && "$currentPropertyVisibility" == "private" ]]; then
			...
			if [[ $multiExpression == 'true' ]] && [[ "$1" == '{' ]]; then
				if [[ $bracketsStarted == 'true' ]]; then
					## handle edge case of '}' as actual parameter
					while ((closingBracketCount += -1)); do
						params+=('}')
					done
				...
				# mode=params
			elif [[ $multiExpression == 'true' ]] && [[ "$1" == '}' ]]; then
				...
				if Variable::Exists __${typeSanitized}_property_names \
					&& @get __${typeSanitized}_property_names | __return_self_and_result=false array.contains $1; then
					...
					if [[ $index -ge 0 ]]; then
						...
						if [[ -z "${!propertyValueIndirect}" && "$typeParam" =~ [aA] ]]; then
		...
		elif [[ "$prevMode" == 'property' ]]; then
			if [[ "$currentPropertyVisibility" == 'public' || "$__access_private" == "true" ]]; then
		...
		if [[ ${#propertyTree[@]} -gt 1 ]]; then
			...
			local property
			local parent
			for parent in "${reversedPropertyTree[@]}"; do
